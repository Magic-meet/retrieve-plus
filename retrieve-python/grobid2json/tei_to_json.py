...
from .s2orc import Paper

from .utils.grobid_util import parse_bib_entry, extract_paper_metadata_from_grobid_xml
from .utils.citation_util import SINGLE_BRACKET_REGEX, BRACKET_REGEX, BRACKET_STYLE_THRESHOLD
from .utils.citation_util import is_expansion_string, _clean_empty_and_duplicate_authors_from_grobid_parse
from .utils.refspan_util import sub_spans_and_update_indices
...
def extract_keywords_from_tei_xml(sp: BeautifulSoup) -> List[str]:
    """
    Extract keywords from TEI XML soup
    :param sp: BeautifulSoup object representing TEI XML
    :return: List of keywords
    """
    keywords = []

    # Find keywords within textClass
    text_class = sp.profileDesc.textClass
    if text_class and text_class.keywords:
        for term in text_class.keywords.find_all('term'):
            if term.text:
...
def convert_tei_xml_soup_to_s2orc_json(soup: BeautifulSoup, paper_id: str, pdf_hash: str) -> Paper:
    ...
    keywords = extract_keywords_from_tei_xml(soup)
